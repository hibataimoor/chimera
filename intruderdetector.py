...
def call_using_twilio():
    # Function to make a call using Twilio
    client = Client(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
    call = client.calls.create(
        to=RECIPIENT_PHONE_NUMBER,
        from_=TWILIO_PHONE_NUMBER,
		twiml='<Response><Say>This is an alert. There is a person with no valid passcode in the building.</Say></Response>'
    )
...
def people_counter():
	...
	if not args.get("input", False):
		...
		vs = VideoStream(src=0).start()  # This makes vs use the default webcam
		time.sleep(2.0)
			...
			for i in np.arange(0, detections.shape[2]):
				...
				if confidence > args["confidence"]:
					...
					if CLASSES[idx] == "person":
						speak_message("Person detected. Please say the passcode.")
						
						for attempt in range(2):  # Give the user two attempts
							passcode = get_voice_input()
							if passcode == "1256":
								speak_message("Welcome!")
								print("Welcome!")
								break
							elif attempt == 1:  # If it's the second attempt and fails
								speak_message("Passcode incorrect. Initiating call.")
								print("Passcode incorrect. Initiating call...")
								call_using_twilio()
								break
						else:
							speak_message("No valid passcode entered. Triggering alert.")
							print("No valid passcode entered. Triggering alert.")
							call_using_twilio()
							break
					...
					# to establish movement and direction of the person
					if oldX > startX:
						print("The person is moving right.")
					else:
						print("The person is moving left.")
					oldX = startX
					if oldY > startY:
						print("The person is moving forwards.")
					else:
						print("The person is moving backwards.")
					oldY = startY

					#  Add Command Velocity Message publishing Here 
						#  publish Person ID as well Detection message.
